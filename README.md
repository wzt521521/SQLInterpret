...
Windows 下，双击项目根目录的 **`启动MiniDBMS.vbs`**，直接打开本地窗口，无需浏览器或输入终端命令。当前电脑已经完成环境安装和 C++ 编译。启动器使用项目内的 `.venv` 环境；复制到另一台电脑时，需要先按下方说明安装 Python 环境并编译 C++ 模块。

也可通过命令启动：

```powershell
.\.venv\Scripts\python.exe -m minidbms.desktop
```

打开后点击「执行 SQL」，即可运行预填的建表、插入、查询和删除演示。「载入演示」每次使用新的表名，可重复演示。点击「错误示例」→「执行 SQL」，查看错误信息、行列号和高亮字符，点击「定位到错误」跳转编辑器。支持 Ctrl+Enter 执行、编辑器右键复制粘贴、打开和保存 SQL 文件、切换数据库以及结果表格右键复制。

结果按语句分标签页显示，底部展示本次执行的缓存统计。默认数据库为项目目录的 `desktop_demo.db`，成功执行的修改会保存；中途失败时展示已完成的结果，之前的修改不会回滚。语法解析失败时不会开始执行。没有精确源码位置的执行错误只定位到语句起始位置，并在界面注明。SQL 修改后需重新执行以更新定位。

关闭窗口即可退出，再次双击启动即可读取原数据库。执行过程中会等待操作完成后再允许关闭窗口，确保正常保存。界面使用 Python 自带的 Tkinter，不需要启动网页服务。
